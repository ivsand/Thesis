...
public class UIC_LineRenderer : Graphic
{
    ...
    void DrawCap(Vector2 position, float angle, float size, UIC_Line.CapTypeEnum type, Color color)
    {
    ...
    void DrawCircle(Vector2 position, float radius, Color color)
    {
        ...
        for (int j = 0; j <= 10; j++)
        {
            ...
            _vh.AddUIVertexQuad(SetVbo(new[] { pos0, pos1, pos2, pos3 }, new[] { uv0, uv1, uv2, uv3 }, color));
        }
    }
}
